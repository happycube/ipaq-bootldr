 ...
 *		(John Hall (john.hall@optionexist.co.uk))
 *
 * $Id: nand.c,v 1.1 2003/04/08 13:24:46 jamey Exp $
 *
...
#include <errno.h>
#include <nand/nand.h>
#include <nand/nand_ids.h>
#include <nand/nand_ecc.h>
#define CONFIG_MTD_NAND_VERIFY_WRITE
#define	MTD_DEBUG_LEVEL0	"0"
#define	MTD_DEBUG_LEVEL3	"3"
//#define	DEBUG(X,Y,...) (X==MTD_DEBUG_LEVEL0) ? putstr(Y##"\r\n") : putstr("")
#define	DEBUG(X);
...
#else
static int nand_erase (struct mtd_info *mtd, loff_t from, size_t len);
#endif
...
int nand_wait(struct mtd_info *mtd, struct nand_chip *this, int state)
{
...
DEBUG("nand_write_page: NAND_ECC_SOFT\r\n");
...
DEBUG("nand_write_page: NAND_ECC_NONE\r\n");
	}			
...
//		BUG();	
DEBUG("nand_write_page: Invalid NAND_ECC_MODE\r\n");
	}	
	
	/* Write out OOB data */
	for (i = 0; i <  mtd->oobsize; i++) {
		writeb ( oob_data[i] , this->IO_ADDR_W);
//putLabeledWord("Writing oob 0x",oob_data[i]);
	}
...
DEBUG("nand_write_page: write failed\r\n");
		return -EIO;
	}
...
DEBUG("nand_write_page: Verifying\r\n");
	/* Send command to read back the page */
	this->cmdfunc (mtd, NAND_CMD_READ0, col, page);
	/* Loop through and verify the data */
	for (i = col; i < last; i++) {
//		if (this->data_buf[i] != readb (this->IO_ADDR_R)) {
//	    unsigned char r= readb(this -> IO_ADDR_R);
//		if (this->data_poi[i] != r) {
//		if (this->data_buf[i] != r) {
		if (this->data_poi[i] != readb (this->IO_ADDR_R)) {
//			DEBUG (MTD_DEBUG_LEVEL0, "%s: " "Failed write verify, page 0x%08x ", __FUNCTION__, page);
DEBUG("nand_write_page: Verify data failure\r\n");
//putLabeledWord("Data error index = 0x",i);
//putLabeledWord("Data expected = 0x",this->data_buf[i]);
//putLabeledWord("Data expected = 0x",this->data_poi[i]);
//putLabeledWord("Data found    = 0x",r);
			return -EIO;
		}
	}

	/* check, if we have a fs-supplied oob-buffer */
DEBUG("nand_write_page: Verifying ecc\r\n");
	if (oob_buf) {
DEBUG("nand_write_page: Verifying oob_buf\r\n");
		for (i = 0; i < mtd->oobsize; i++) {
			if (oob_data[i] != readb (this->IO_ADDR_R)) {
//				DEBUG (MTD_DEBUG_LEVEL0, "%s: " "Failed write verify, page 0x%08x ", __FUNCTION__, page);
DEBUG("nand_write_page: Verify oob failure\r\n");
				return -EIO;
			}
		}
	} else {
...
DEBUG("nand_write_ecc: Attempt to write past end of page\r\n");
		return -EINVAL;
	}
...
DEBUG("nand_write_ecc: Write protected!!\r\n");
		ret = -EIO;
		goto out;
	}
	...
	while (written < len) {
		...
		if (ret) {
DEBUG("nand_write_ecc: nand_write_page failed\r\n");
			goto out;
		}
...
//		DEBUG (MTD_DEBUG_LEVEL0, "nand_write_oob: Attempt to write past end of page\n");
DEBUG("nand_write_oob: Attempt to write past end of page\r\n");
		return -EINVAL;
	}
...
DEBUG("nand_write_oob: Device is write protected\r\n");
		ret = -EIO;
		goto out;
	}
...
DEBUG("nand_write_oob: Failed write\r\n");
		ret = -EIO;
		goto out;
	}
	/* Return happy */
...
DEBUG("nand_write_oob: Failed write verify\r\n");
			ret = -EIO;
			goto out;
		}
	}
...
/*
 * NAND erase a block - new style
 */
static int nand_erase (struct mtd_info *mtd, loff_t from, size_t length)
{
	int page, len, status, pages_per_block, ret;
	struct nand_chip *this = mtd->priv;
#if 0
...
//		DEBUG (MTD_DEBUG_LEVEL0, "nand_erase: Unaligned address\n");
		DEBUG("nand_erase: erase block does not start on an erase boundary\r\n");
		return -EINVAL;
	}
...
//		DEBUG (MTD_DEBUG_LEVEL0, "nand_erase: Length not block aligned\n");
		DEBUG("nand_erase: erase block not aligned\r\n");
		return -EINVAL;
	}

	/* Do not allow erase past end of device */
//	if ((instr->len + instr->addr) > mtd->size) {
	if ((length + from) > mtd->size) {
//		DEBUG (MTD_DEBUG_LEVEL0, "nand_erase: Erase past end of device\n");
		DEBUG("nand_erase: erase size too long\r\n");
		return -EINVAL;
	}
...
//	instr->state = MTD_ERASING;
	...
	while (len) {
		...
		if (force_erase)
		    ;
		else if (readb (this->IO_ADDR_R) != 0xff) {
//			printk (KERN_WARNING "nand_erase: attempt to erase a bad block at page 0x%08x\n", page);
//			instr->state = MTD_ERASE_FAILED;
			goto erase_exit;
		}
...
#endif
			putLabeledWord("nand_maf_id = 0x",nand_maf_id);
			putLabeledWord("nand_dev_id = 0x",nand_dev_id);
			putstr("nand device name: "); putstr(mtd->name); putstr("\r\n");
...
//		printk (KERN_WARNING "Invalid NAND_ECC_MODE %d\n", this->eccmode);
//		BUG();	
putstr("nand_scan: Invalid NAND_ECC mode\r\n");
	}	
#else
	this->eccmode = NAND_ECC_NONE;
#endif
