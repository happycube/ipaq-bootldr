...
void showMMUEntry(unsigned long sectionNumber)
{
    putHexInt32(sectionNumber);
    putstr(" ");
    putHexInt32(mmu_table[sectionNumber]);
    putstr("\r\n");
}    


#if 1
#define PXA_BUFFERABLE 0		// for buggy pxa250
#else
#define PXA_BUFFERABLE MMU_BUFFERABLE
#endif
...
void bootConfigureMMU(void)
{
  ...
  if ((char*)&_start == (char*)FLASH_BASE) {
    ...
    putLabeledWord("PXA: Evacuating 1MB of Flash to DRAM at: ", FLASH_IMG_START);
